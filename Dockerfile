...
RUN git clone --depth 1 --single-branch --branch $TOOL_VERSION https://github.com/rgcca-factory/$TOOL_NAME && \
    git clone --depth 1 --single-branch --branch master https://github.com/BrainAndSpineInstitute/$TOOL_NAME_UI && \
    cd $TOOL_NAME && \
	...
	mkdir /inst/ && \
	cp -r $TOOL_NAME/R/ / && \
	cp -r $TOOL_NAME_UI/inst/extdata/ data && \
	mv $TOOL_NAME_UI/inst/launcher.R inst/launcher.R && \
	rm -rf /var/lib/{cache,log}/ /tmp/* /var/tmp/* $TOOL_NAME $TOOL_NAME_UI
